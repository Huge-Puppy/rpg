...
public class BattleSystem : MonoBehaviour
{
    ...
    [SerializeField] VectorVariable playerPosition;
    [SerializeField] VectorVariable respawnPosition;
    ...
    public IEnumerator SetupBattle()
    {
        ...
        yield return StartCoroutine(dialogBox.TypeDialog($"A dangerous {currentEnemy.value.enemyName} approaches!"));
    ...
    IEnumerator EnemyTurn()
    {
        ...
        yield return dialogBox.TypeDialog($"{currentEnemy.value.enemyName} attacks!");
        yield return new WaitForSecondsRealtime(textPause);
        float damageDone = attackController.EnemyAttack();
        yield return dialogBox.TypeDialog($"{currentEnemy.value.enemyName} does {damageDone} damage.");
        yield return new WaitForSecondsRealtime(textPause);
    ...
    IEnumerator HandleBattleEnd()
    {
        ...
        if (newState == BattleState.PlayerWon)
        {
            ...
            yield return new WaitForSecondsRealtime(textPause);
            yield return dialogBox.TypeDialog($"You gained {currentEnemy.value.xp} xp and {currentEnemy.value.gold} gold.");
            yield return new WaitForSecondsRealtime(textPause);
            playerStats.gold.value += currentEnemy.value.gold;

            // #if UNITY_WEBGL == true && UNITY_EDITOR == false
            // GiveGold(currentEnemy.enemy.gold);
            // #endif
            playerStats.xp.value += currentEnemy.value.xp;
